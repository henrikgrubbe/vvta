    ...
    <main class="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div class="max-w-sm w-full bg-white rounded-2xl shadow-md p-8 space-y-6">
        <div class="text-center">
          ...
          <h1 class="mt-3 text-2xl font-bold text-gray-900">{{ 'onboarding.welcome' | translate }}</h1>
          <p class="mt-1 text-gray-500 text-sm">{{ 'onboarding.subtitle' | translate }}</p>
        </div>
        ...
        <form (submit)="save(); $event.preventDefault()" novalidate class="space-y-4">
          <div>
            ...
            @if (touched() && !firstName().trim()) {
              <p id="name-error" class="text-red-600 text-sm mt-1" role="alert">
                {{ 'onboarding.nameError' | translate }}
              </p>
            }
          </div>
          ...
          >
            {{ saving() ? ('onboarding.saving' | translate) : ('onboarding.getStarted' | translate) }}
          </button>
        </form>
      </div>
    </main>
...
export class OnboardingComponent {
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      this.error.set(err instanceof Error ? err.message : this.i18n.get('onboarding.genericError'));
    } finally {
